...
网页上除了切换工作状态，还提供完整的 Pixoo64 控制面板：

| 功能 | 说明 |
| --- | --- |
| 设备控制 | 亮度、屏幕开关、旋转角度、镜像、白平衡、高亮模式、小时制、温度单位，改动即时下发 |
| 自动亮度 / 夜间关屏 | 设置白天/夜间亮度和时间段，可让夜间完全熄屏，每 30 秒自动同步 |
| 滚动文字通知 | 在屏幕上滚动显示一段文字，到期自动恢复状态图案（中文/英文均可） |
| 倒计时 | 调用设备内置倒计时工具，结束后自动回到状态图案 |
| 秒表 | 设备内置秒表 开始 / 停止 / 复位 |
| 比分板 | 红蓝两队比分 +1 / -1 / 归零 |
| 蜂鸣器 | 短响 / 长响提醒（注意别吓到人） |
| 设备发现 | 通过 Divoom 云端自动发现同一局域网里的 Pixoo，不用手填 IP |
| 天气 | 读取设备内置天气（需先在 Divoom App 里绑定城市） |

新增 API：`POST /api/config`（设备控制/自动模式）、`/api/notify`、
`/api/countdown`、`/api/stopwatch`、`/api/scoreboard`、`/api/buzzer`、
`/api/discover`、`/api/mode`，以及 `GET /api/weather`。
